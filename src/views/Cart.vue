        ...
        <div class="cart-items-wrapper bg-white rounded shadow-sm mb-3">
          <div v-for="item in cartItems" :key="item.variantId" class="cart-item-row d-flex align-items-center py-3 px-4 border-bottom">
            ...
            <div class="col-product d-flex align-items-center pe-3">
              <img :src="item.imageUrl || 'https://ui-avatars.com/api/?name=Product&background=random'" alt="img" class="border rounded me-3" style="width: 80px; height: 80px; object-fit: cover;">
            ...
            <div class="col-price text-center text-muted">{{ formatPrice(item.unitPrice) }}</div>
          ...
          <div class="text-end me-4">
            <div class="d-flex align-items-center">
              ...
              <div class="d-flex flex-column align-items-end">
                <span v-if="discountAmount > 0" class="text-muted text-decoration-line-through small">{{ formatPrice(selectedTotalAmount) }}</span>
                <span class="fs-4 fw-bold" style="color: #ee4d2d;">{{ formatPrice(finalTotalAmount) }}</span>
              </div>
            </div>
          </div>
        ...
        <div class="modal-content border-0">
          ...
          <div class="modal-body">
            <div class="d-flex gap-2 mb-4 bg-white p-3 rounded shadow-sm border">
              <input type="text" v-model="voucherCodeInput" class="form-control border-0 shadow-none" placeholder="Mã Voucher">
              <button class="btn btn-danger fw-bold px-4 py-2 text-nowrap" @click="applyVoucherByCode">ÁP DỤNG</button>
            </div>

            <h6 class="mb-3 fw-bold">Kho Voucher của bạn</h6>
            <div class="voucher-list px-1" style="max-height: 400px; overflow-y: auto;">
              <div v-if="vouchers.length === 0" class="text-center py-5 text-muted">Bạn chưa có mã giảm giá nào</div>
              ...
              <div v-for="v in vouchers" :key="v.id" 
                ...
                <div class="bg-danger text-white text-center p-3 d-flex flex-row align-items-center justify-content-center" style="width: 120px;">
                  <div class="d-flex flex-column">
                    <span class="small text-uppercase" style="font-size: 0.7rem;">Giảm</span>
                    <span class="fw-bold fs-4">
                      {{ v.discountType === 'PERCENTAGE' ? v.discountValue + '%' : formatShortPrice(v.discountValue) }}
                    </span>
                  </div>
                </div>

                <div class="p-3 flex-grow-1 bg-white border-start border-3 border-danger border-opacity-10">
                  <div class="fw-bold text-dark truncate-2">{{ v.name }}</div>
                  <div class="small text-muted mt-1">Đơn tối thiểu: {{ formatPrice(v.minOrderValue) }}</div>
                  
                  <div class="mt-2 d-flex align-items-center justify-content-between">
                    <span class="text-danger small" style="font-size: 0.75rem;">
                      HSD: {{ formatDate(v.endDate) }}
                    </span>
                    <span v-if="selectedTotalAmount < v.minOrderValue" class="badge bg-secondary" style="font-size: 0.6rem;">
                      Thiếu {{ formatPrice(v.minOrderValue - selectedTotalAmount) }}
                    </span>
                  </div>
                </div>

                <div class="p-3 bg-white d-flex align-items-center">
                  <input type="radio" name="v-radio" :value="v" v-model="tempSelectedVoucher" 
          ...
          <div class="modal-footer border-0">
            ...
            <button type="button" class="btn btn-danger px-4 fw-bold" data-bs-dismiss="modal" @click="confirmVoucher">XÁC NHẬN</button>
          </div>
        </div>
      </div>
    </div>
...
export default {
  ...
  computed: {
    selectAll: {
      get() { return this.cartItems.length > 0 && this.selectedItems.length === this.cartItems.length; },
      set(value) { this.selectedItems = value ? this.cartItems.map(item => item.variantId) : []; }
    },
    selectedTotalAmount() {
    ...
    discountAmount() {
      if (!this.appliedVoucher || this.selectedTotalAmount < this.appliedVoucher.minOrderValue) return 0;
      const v = this.appliedVoucher;
      if (v.discountType === 'PERCENTAGE') {
        let amount = (this.selectedTotalAmount * v.discountValue) / 100;
        return v.maxDiscountAmount ? Math.min(amount, v.maxDiscountAmount) : amount;
      }
  ...
  watch: {
    cartItems: {
      deep: true,
      handler(newItems) { cartCount.value = newItems.reduce((sum, item) => sum + item.quantity, 0); }
    }
  },
  methods: {
    getAuthHeaders() {
  ...
  try {
    ...
    async fetchCart() {
      ...
      async fetchUserVouchers() {
        ...
        try {
          const res = await axios.get(`http://localhost:8080/promotions/my-vouchers/${userId}`, {
    ...
    async applyVoucherByCode() {
      ...
      try {
        const userId = this.getUserId();
        await axios.post(`http://localhost:8080/promotions/claim`, {
          userId: userId,
        ...
        alert(e.response?.data?.error || "Mã không hợp lệ hoặc đã được sử dụng");
      }
    },
    confirmVoucher() { this.appliedVoucher = this.tempSelectedVoucher; },
    formatPrice(p) {
      return new Intl.NumberFormat("vi-VN", { style: "currency", currency: "VND" }).format(p || 0).replace('VND', '₫');
    },
    ...
    increaseQty(item) { item.quantity += 1; this.updateItem(item); },
    decreaseQty(item) { if (item.quantity > 1) { item.quantity -= 1; this.updateItem(item); } },
    async updateItem(item) {
      try {
        await axios.put("http://localhost:8080/cart/item", { variantId: item.variantId, quantity: item.quantity }, { headers: this.getAuthHeaders() });
    ...
    async removeSelectedItems() {
      if (!this.selectedItems.length || !confirm("Xóa các mục đã chọn?")) return;
      try {
        for (const id of this.selectedItems) {
          await axios.delete(`http://localhost:8080/cart/item/${id}`, { headers: this.getAuthHeaders() });
        }
        this.selectedItems = [];
        this.fetchCart();
      } catch (e) { alert("Lỗi xóa nhiều!"); }
    },
    checkout() {
      if (!this.selectedItems.length) return alert("Chọn sản phẩm!");
      const selectedCartItems = this.cartItems.filter(i => this.selectedItems.includes(i.variantId));
      const order = {
        totalAmount: this.finalTotalAmount,
        discountAmount: this.discountAmount,
        voucherId: this.appliedVoucher?.id || null,
        items: selectedCartItems.map(i => ({
          externalItemId: i.variantId,
      ...
      this.$router.push({ path: "/orders", state: { order } });
    }
  },
  mounted() { this.fetchCart(); }
};
</script>

<style scoped>
.cart-page-wrapper { background-color: #f5f5f5; min-height: 100vh; padding-bottom: 100px; }
.col-checkbox { width: 5%; }
.col-product { width: 45%; }
.col-price, .col-qty { width: 15%; }
.col-total, .col-action { width: 10%; }
.brand-checkbox { width: 1.2rem; height: 1.2rem; cursor: pointer; border-radius: 2px; }
.brand-checkbox:checked { background-color: #ee4d2d; border-color: #ee4d2d; }
.checkout-sticky-bar { position: fixed; bottom: 0; left: 0; width: 100%; z-index: 1000; border-top: 1px solid #eee; }
.voucher-list::-webkit-scrollbar { width: 5px; }
.voucher-list::-webkit-scrollbar-thumb { background: #ddd; border-radius: 10px; }
